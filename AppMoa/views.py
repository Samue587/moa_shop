...
def registro_view(request):
    if request.method == "POST":
        ...
        if Usuario.objects.filter(correo_usuario=email).exists():
            return render(
                request, "registro.html", {"error": "El email ya está registrado."}
            )
...
def tienda(request):
    ...
    productos = (
        Producto.objects.select_related("categoria", "catalogo", "proveedor")
        .filter(estado="DISPONIBLE")
        .annotate(
            tallas_con_stock=Count(
                "variaciones", filter=Q(variaciones__stock_actual__gt=0)
            )
        )
    )
...
def detalle_producto(request, id):
    producto = get_object_or_404(
        Producto.objects.select_related("categoria", "proveedor"), pk=id
    )
    variaciones = VariacionProducto.objects.filter(
        producto=producto, stock_actual__gt=0
    )
    return render(
        request,
        "tienda/detalle.html",
...
def carrito_agregar(request):
    ...
    try:
        variacion = VariacionProducto.objects.select_related("producto").get(
            pk=variacion_id
        )
    ...
    carrito = _get_carrito(request)
    item_existente = next(
        (i for i in carrito["items"] if i["variacion_id"] == variacion.id), None
    )
    cant_actual = item_existente["cantidad"] if item_existente else 0

    if variacion.stock_actual < (cant_actual + cantidad):
        messages.error(
            request, f"Stock insuficiente. Disponibles: {variacion.stock_actual}."
        )
        return redirect("tienda")
    ...
    messages.success(
        request, f'"{variacion.producto.nombre_producto}" agregado al carrito.'
    )
...
def carrito_remover(request, variacion_id):
    ...
    carrito = _get_carrito(request)
    carrito["items"] = [
        i for i in carrito["items"] if i["variacion_id"] != int(variacion_id)
    ]
    _save_carrito(request, carrito)
...
def perfil_usuario(request):
    ...
    ventas = (
        ...
        .prefetch_related("detalles__variacion__producto", "envio")
        .order_by("-id")
    )
        ...
        if accion == "actualizar_datos":
            ...
            if not nombres or not email:
                ...
                return render(
                    request,
                    "tienda/perfil.html",
                    {"usuario": usuario, "ventas": ventas},
                )

            if (
                Usuario.objects.filter(correo_usuario=email)
                .exclude(pk=usuario.id)
                .exists()
            ):
            ...
            if len(password_nuevo) < 6:
                messages.error(
                    request, "La nueva contraseña debe tener al menos 6 caracteres."
                )
                return render(
                    request,
                    "tienda/perfil.html",
...
def panel_admin(request):
    ...
    for i in range(11, -1, -1):
        ...
        count = Venta.objects.filter(
            fecha__date__gte=mes_inicio, fecha__date__lt=mes_fin
        ).count()
        ventas_12m.append(count)
        labels_12m.append(mes_inicio.strftime("%b"))

    # Ventas por categoría
    ventas_cat = (
        DetalleVenta.objects.values("variacion__producto__categoria__nombre")
        .annotate(total=Count("id"))
        .order_by("-total")[:5]
    )
    cat_labels = [
        v["variacion__producto__categoria__nombre"] or "Sin categoría"
        for v in ventas_cat
    ]
    cat_data = [v["total"] for v in ventas_cat]
    ...
    ingresos_mes = (
        Venta.objects.filter(fecha__date__gte=primer_dia_mes).aggregate(
    ...
    # Últimos usuarios
    ultimos_usuarios = Usuario.objects.order_by("-creado_en")[:5]

    # Top productos más vendidos
    top_raw = (
        DetalleVenta.objects.values("variacion__producto__nombre_producto")
        .annotate(ventas=Sum("cantidad"))
        .order_by("-ventas")[:5]
    )
    max_v = top_raw[0]["ventas"] if top_raw else 1
    top_productos = [
        {
            "nombre": t["variacion__producto__nombre_producto"],
            "ventas": t["ventas"],
            "pct": int(t["ventas"] / max_v * 100),
        }
        for t in top_raw
    ]

    # Stock bajo
    variaciones_bajo_stock = (
        VariacionProducto.objects.select_related("producto")
        .filter(stock_actual__lte=5)
        .order_by("stock_actual")[:6]
    )

    # Estado envíos
    envios_estado = {
        "pendientes": Envio.objects.filter(estado_envio="PENDIENTE").count(),
        "en_camino": Envio.objects.filter(estado_envio="EN_CAMINO").count(),
        "entregados": Envio.objects.filter(estado_envio="ENTREGADO").count(),
    }
    ...
    context = {
        ...
        or 0,
        "productos_recientes": Producto.objects.select_related("categoria").order_by(
            "-creado_en"
        )[:8],
        "ventas_recientes": ultimas_ventas,
        "envios_pendientes": Envio.objects.exclude(
            estado_envio__in=["ENTREGADO", "CANCELADO"]
        ).count(),
        "stock_bajo": sum(1 for v in VariacionProducto.objects.all() if v.bajo_stock),
        ...
        "variaciones_bajo_stock": variaciones_bajo_stock,
        "envios_estado": envios_estado,
        "fecha_hoy": fecha_hoy,
    }
...
def listar_roles(request):
    ...
    return render(
        request, "admin/roles/lista.html", {"roles": roles, "usuario": request.usuario}
    )


def asignar_permisos():

    admin = Rol.objects.get(nombre_rol="Administrador")
...
def crear_rol(request):
            ...
            for pid in permisos_ids:
                try:
                    RolPermiso.objects.create(
                        rol=rol, permiso=Permiso.objects.get(pk=pid)
                    )
    ...
    return render(
        request,
        "admin/roles/crear.html",
        {"form": {}, "permisos": permisos, "usuario": request.usuario},
    )
...
def editar_rol(request, id):
    ...
    permisos = Permiso.objects.all().order_by("slug")
    permisos_del_rol = RolPermiso.objects.filter(rol=rol).values_list(
...
def crear_permiso(request):
    ...
    return render(
        request, "admin/permisos/crear.html", {"form": {}, "usuario": request.usuario}
    )
...
def editar_usuario(request, id):
    ...
    if request.method == "POST":
        ...
        if (
            str(usr.id) == str(request.session.get("usuario_id"))
            and estado == "INACTIVO"
        ):
            request.session.flush()
            return redirect("login")
...
def editar_proveedor(request, id):
    ...
    if request.method == "POST":
        ...
        nit = (
            request.POST.get("nit_proveedor", "")
            .strip()
            .replace(".", "")
            .replace(",", "")
            .replace(" ", "")
        ...
        nombre = request.POST.get("nombre_proveedor", "").strip()
        telefono = (
            request.POST.get("telefono_proveedor", "")
            .strip()
            .replace(".", "")
            .replace(",", "")
            .replace(" ", "")
        ...
        try:
            ...
            proveedor.contacto_proveedor = (
                request.POST.get("contacto_proveedor", "").strip() or None
            )
            proveedor.direccion_proveedor = (
                request.POST.get("direccion_proveedor", "").strip() or None
            )
            proveedor.observaciones_proveedor = (
                request.POST.get("observaciones_proveedor", "").strip() or None
            )
            proveedor.save()
...
def eliminar_proveedor(request, id):
    ...
    return render(
        request,
        "admin/proveedores/eliminar.html",
...
def crear_catalogo(request):
    ...
    return render(
        request, "admin/catalogos/crear.html", {"form": {}, "usuario": request.usuario}
    )
...
def crear_categoria(request):
    ...
    return render(
        request, "admin/categorias/crear.html", {"form": {}, "usuario": request.usuario}
    )
...
def eliminar_categoria(request, id):
    ...
    if request.method == "POST":
        tiene_historial = (
            ...
            or DetalleEntrada.objects.filter(
                variacion__producto__categoria=categoria
            ).exists()
        )
        ...
        messages.success(
            request,
            f'Categoría "{nombre}" y sus {productos_count} producto(s) eliminados.',
        )
        return redirect("admin_categorias")

    return render(
        request,
        "admin/categorias/eliminar.html",
        {
            "categoria": categoria,
    ...
    if request.method == "POST":
        ...
        try:
            ...
            Producto.objects.create(
                ...
                catalogo=(
                    Catalogo.objects.filter(pk=catalogo_id).first()
                    if catalogo_id
                    else None
                ),
                foto=foto,
...
def editar_producto(request, id):
    producto = get_object_or_404(
        Producto.objects.select_related("categoria", "proveedor", "catalogo"), pk=id
    )
    categorias = Categoria.objects.filter(estado="ACTIVO")
    proveedores = Proveedor.objects.all()
    catalogos = Catalogo.objects.all()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            producto.catalogo = (
                Catalogo.objects.filter(pk=catalogo_id).first() if catalogo_id else None
            )
...
def eliminar_producto(request, id):
    ...
    if request.method == "POST":
        tiene_ventas = DetalleVenta.objects.filter(
            variacion__producto=producto
        ).exists()
        tiene_entradas = DetalleEntrada.objects.filter(
            variacion__producto=producto
        ).exists()
...
def listar_variaciones(request, producto_id):
    ...
    variaciones = VariacionProducto.objects.filter(producto=producto).order_by(
        "talla", "color"
    )
    return render(
        request,
        "admin/variaciones/lista.html",
...
def eliminar_variacion(request, producto_id, id):
    ...
    return render(
        request,
        "admin/variaciones/eliminar.html",
...
def eliminar_entrada(request, id):
    ...
    return render(
        request,
        "admin/inventario/eliminar.html",
...
def listar_ventas(request):
    ...
    ventas = (
        ...
        .prefetch_related("detalles__variacion__producto")
        .order_by("-id")
    )
...
def exportar_ventas_excel(request):
    ...
    header_fill = PatternFill(
        start_color="1447A8", end_color="1447A8", fill_type="solid"
    )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="ventas_{timezone.now().date()}.xlsx"'
    )
    wb.save(response)
    return response
...
def exportar_ventas_pdf(request):
    ...
    estilo_titulo = ParagraphStyle(
        "titulo", fontName="Helvetica-Bold", fontSize=20, textColor=DARK, spaceAfter=2
    )
    ...
    elements.append(
        Paragraph(f"Generado el {now_str}  ·  Colombia, Bogotá D.C.", estilo_sub)
    )
    elements.append(
        HRFlowable(
            width="100%", thickness=3, color=NARANJA, spaceBefore=8, spaceAfter=10
        )
    )
    ...
    elements.append(
        HRFlowable(
            width="100%", thickness=0.5, color=GRIS2, spaceBefore=4, spaceAfter=8
        )
    )
    elements.append(
        Paragraph("© 2026 TiendaMoa07  ·  Colombia, Bogotá D.C.", estilo_pie)
    )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="ventas_{timezone.now().strftime("%d%m%Y")}.pdf"'
    )
    return response
...
def editar_envio(request, id):
    ...
    if request.method == "POST":
        ...
        if (
            numero_guia
            and Envio.objects.filter(numero_guia=numero_guia).exclude(pk=id).exists()
        ):

            messages.error(
                request, f'Ya existe otro envío con número de guía "{numero_guia}".'
            )
        ...
        envio.especificaciones_llegada = (
            especificaciones or envio.especificaciones_llegada
        )
        ...
        envio.fecha_estipulada_llegada = fecha_llegada or envio.fecha_estipulada_llegada

        envio.costo_envio = Decimal(costo)

        envio.save()

        messages.success(request, f"Envío #{id} actualizado correctamente.")
...
def envio_cambiar_estado(request, id):
    if request.method == "POST":
        ...
        print("DESPUÉS:", envio.estado_envio)
        
        numero_guia = request.POST.get("numero_guia", "").strip()
    empresa_transportadora = request.POST.get(
    ...
    if empresa_transportadora:
        envio.empresa_transportadora = empresa_transportadora
        envio.estado_envio = nuevo_estado
        envio.save()
...
def reporte_inventario(request):
    ...
    productos_mayor_rotacion = [
        {
            "nombre": x["variacion__producto__nombre_producto"],
    ...
    total_mayor_rotacion = (
        productos_mayor_rotacion[0]["unidades_vendidas"]
        if productos_mayor_rotacion
        else 0
    )

    # Filtros disponibles
    tallas_disponibles = (
        VariacionProducto.objects.values_list("talla", flat=True)
        .distinct()
        .order_by("talla")
    )
    colores_disponibles = (
        VariacionProducto.objects.values_list("color", flat=True)
        .distinct()
        .order_by("color")
    )
...
def reporte_ventas(request):
    ...
    top_usuarios = (
        Venta.objects.values(
            "usuario__nombres_usuario",
            "usuario__apellidos_usuario",
            "usuario__correo_usuario",
        )
    ...
    ventas_por_dia = (
        Venta.objects.filter(fecha__date__gte=desde_30)
...
def reporte_categorias(request):
    ...
    if fecha_inicio and fecha_fin:
        detalles_qs = detalles_qs.filter(
            venta__fecha__date__gte=fecha_inicio, venta__fecha__date__lte=fecha_fin
        )
    ...
    return render(
        request,
        "admin/reportes/categorias.html",
        {
...
def reporte_envios(request):
    ...
    if fecha_inicio and fecha_fin:
        envios = envios.filter(
            fecha_envio__date__gte=fecha_inicio, fecha_envio__date__lte=fecha_fin
        )
    ...
    top_empresas = (
        Envio.objects.values("empresa_transportadora")
        .annotate(total=Count("id"))
        .order_by("-total")[:6]
    )
    ...
    # Top destinos
    top_destinos = (
        Envio.objects.values("ciudad_envio", "departamento_envio")
        .annotate(total=Count("id"))
        .order_by("-total")[:8]
    )

    empresas_disponibles = (
        Envio.objects.values_list("empresa_transportadora", flat=True)
        .distinct()
...
def exportar_clientes_excel(request):
    ...
    clientes = Usuario.objects.annotate(
        num_ventas=Count("ventas", distinct=True),
        total_gastado=Sum("ventas__monto_final"),
    ).order_by("-num_ventas")

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Clientes MOA"

    headers = ["Cliente", "Email", "N° Compras", "Total Gastado", "Miembro desde"]
    header_fill = PatternFill(
        start_color="1447A8", end_color="1447A8", fill_type="solid"
    )
    header_font = Font(color="FFFFFF", bold=True)
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="clientes_{timezone.now().date()}.xlsx"'
    )
    wb.save(response)
    return response
...
def exportar_clientes_pdf(request):
    ...
    banner = Table([["MOA Moda"]], colWidths=[page_w])
    banner.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), NARANJA),
    ...
    elements.append(banner)
    elements.append(Spacer(1, 10))
    elements.append(Paragraph("Reporte de Clientes", estilo_titulo))
    elements.append(
        Paragraph(f"Generado el {now_str}  ·  Colombia, Bogotá D.C.", estilo_sub)
    )
    elements.append(
    ...
    data = [["#", "Cliente", "Email", "N° Compras", "Total Gastado"]]
    for i, c in enumerate(clientes, 1):
        data.append(
            [
                ...
                c.get_full_name(),
                c.correo_usuario,
                str(c.num_ventas or 0),
                f"${c.total_gastado or 0:,.0f}",
            ]
        )

    col_widths = [12 * mm, 45 * mm, 55 * mm, 22 * mm, 30 * mm]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    elements.append(Paragraph("© 2026 MOA Moda  ·  Colombia, Bogotá D.C.", estilo_pie))

    doc.build(elements)
    buffer.seek(0)
    response = HttpResponse(buffer, content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="clientes_{timezone.now().strftime("%d%m%Y")}.pdf"'
    )
    return response
...
def exportar_envios_excel(request):
    ...
    envios = Envio.objects.select_related("venta__usuario").order_by("-fecha_envio")
    ...
    for e in envios:
        ws.append(
            [
                e.id,
                e.venta.id,
                e.venta.usuario.get_full_name(),
                e.direccion_envio,
                e.ciudad_envio,
                e.departamento_envio,
                e.empresa_transportadora or "",
                e.numero_guia or "",
                e.fecha_envio.strftime("%d/%m/%Y") if e.fecha_envio else "",
                (
                    e.fecha_estipulada_llegada.strftime("%d/%m/%Y")
                    if e.fecha_estipulada_llegada
                    else ""
                ),
                e.estado_envio,
            ]
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="envios_{timezone.now().date()}.xlsx"'
    )
    wb.save(response)
    return response
...
def exportar_envios_pdf(request):
    ...
    elements.append(banner)
    elements.append(Spacer(1, 10))
    elements.append(Paragraph("Reporte de Envíos", estilo_titulo))
    elements.append(
        Paragraph(f"Generado el {now_str}  ·  Colombia, Bogotá D.C.", estilo_sub)
    )
    elements.append(
    ...
    data = [["#", "Cliente", "Ciudad", "Empresa", "Fecha", "Estado"]]
    for i, e in enumerate(envios, 1):
        data.append(
            [
                str(i),
                e.venta.usuario.get_full_name(),
                f"{e.ciudad_envio}, {e.departamento_envio}",
                e.empresa_transportadora or "—",
                e.fecha_envio.strftime("%d/%m/%Y") if e.fecha_envio else "—",
                e.estado_envio,
            ]
    ...
    col_widths = [12 * mm, 45 * mm, 40 * mm, 35 * mm, 22 * mm, 24 * mm]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    response = HttpResponse(buffer, content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="envios_{timezone.now().strftime("%d%m%Y")}.pdf"'
    )
    return response
...
def exportar_inventario_excel(request):
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Inventario MOA"

    headers = ["Producto", "Categoría", "Talla", "Color", "SKU", "Stock", "Estado"]
    header_fill = PatternFill(
        start_color="1447A8", end_color="1447A8", fill_type="solid"
    )
    header_font = Font(color="FFFFFF", bold=True)
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="inventario_{timezone.now().date()}.xlsx"'
    )
    wb.save(response)
    return response
...
def exportar_inventario_pdf(request):
    ...
    elements.append(banner)
    elements.append(Spacer(1, 10))
    elements.append(Paragraph("Reporte de Inventario", estilo_titulo))
    elements.append(
        Paragraph(f"Generado el {now_str}  ·  Colombia, Bogotá D.C.", estilo_sub)
    )
    elements.append(
    ...
    data = [["#", "Producto", "Categoría", "Talla", "Color", "Stock", "Estado"]]
    ...
    col_widths = [10 * mm, 40 * mm, 30 * mm, 15 * mm, 20 * mm, 15 * mm, 24 * mm]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(
        TableStyle(
            [
    ...
    response = HttpResponse(buffer, content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="inventario_{timezone.now().strftime("%d%m%Y")}.pdf"'
    )
    return response
...
def exportar_categorias_excel(request):
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Categorías MOA"

    headers = ["Categoría", "Unidades Vendidas", "N° Ventas", "Total Ingresos"]
    header_fill = PatternFill(
        start_color="1447A8", end_color="1447A8", fill_type="solid"
    )
    header_font = Font(color="FFFFFF", bold=True)
    ...
    for c in por_categoria:
        ws.append(
            [
                c["variacion__producto__categoria__nombre"] or "Sin categoría",
                c["total_unidades"] or 0,
                c["num_ventas"] or 0,
                c["total_ingresos"] or 0,
            ]
        )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="categorias_{timezone.now().date()}.xlsx"'
    )
    wb.save(response)
    return response
...
def exportar_categorias_pdf(request):
    ...
    elements.append(banner)
    elements.append(Spacer(1, 10))
    elements.append(Paragraph("Reporte de Categorías", estilo_titulo))
    elements.append(
        Paragraph(f"Generado el {now_str}  ·  Colombia, Bogotá D.C.", estilo_sub)
    )
    elements.append(
    ...
    data = [["#", "Categoría", "Unidades", "N° Ventas", "Total Ingresos"]]
    for i, c in enumerate(por_categoria, 1):
        data.append(
            [
                str(i),
                c["variacion__producto__categoria__nombre"] or "Sin categoría",
                str(c["total_unidades"] or 0),
                str(c["num_ventas"] or 0),
                f'${c["total_ingresos"] or 0:,.0f}',
            ]
        )

    col_widths = [12 * mm, 55 * mm, 25 * mm, 25 * mm, 35 * mm]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(
        TableStyle(
            [
